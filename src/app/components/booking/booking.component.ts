...
export class BookingComponent implements OnInit {
  ...
  captureTime(index: number) {
    if (this.date == null) {
      ...
      this.dateDb = new Date(
        this.date.slice(0, 4),
        this.mesof,
        this.date.slice(8, 10),
        this.hours[index].hour,
        this.hours[index].minutes
      );
  ...
  loaderBarber(): void {
    this.serviceBarbershop.getbarber(this.idBarbershop).subscribe((data) => {
  ...
  saveBooking() {
    ...
    this.reserveDes = this.compare(this.horareserva, this.fechaActual);
    console.log("indice");
    console.log(this.reserveDes);
    
    console.log(this.reserveDes)
    if ((this.reserveDes == -1)) {
      Swal.fire(
        'informacion',
        'No puedes reservar en una fecha que ya paso',
        'info'
      );
      this.empty();
    } else {
